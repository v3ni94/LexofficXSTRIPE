...
async def _noop_scheduled_loop() -> None:
    pass
...
async def client(db: AsyncSession) -> AsyncGenerator[AsyncClient, None]:
    ...
    with (
        ...
        patch("app.main.scheduled_collections_loop", new=_noop_scheduled_loop),
...
async def test_org(db: AsyncSession) -> Organization:
    ...
    org = Organization(name="Test GmbH")
    db.add(org)
    await db.flush()
...
async def test_user(db: AsyncSession, test_org: Organization) -> User:
    """Tenant A – primary test user (owner of test_org)."""
    user = User(
    ...
    db.add(member)
    db.add(Integration(tenant_id=test_org.id))
    await db.flush()
...
@pytest_asyncio.fixture
async def test_org2(db: AsyncSession) -> Organization:
    """Organization B – for isolation tests."""
    org = Organization(name="Other GmbH")
    db.add(org)
    await db.flush()
    await db.refresh(org)
    return org
...
@pytest_asyncio.fixture
async def test_user2(db: AsyncSession, test_org2: Organization) -> User:
    ...
    db.add(member)
    db.add(Integration(tenant_id=test_org2.id))
    await db.flush()
    await db.refresh(user)
    return user
...
@pytest_asyncio.fixture
async def test_integration(db: AsyncSession, test_user: User, test_org: Organization) -> Integration:
    """Connect both Lexoffice and Stripe for test_org."""
...
@pytest.fixture
def auth_headers():
    """Returns a callable: auth_headers(user_id, org_id, role) -> dict.

    For backward compat, when org_id is not given, tenant_id falls back to user_id.
    """
