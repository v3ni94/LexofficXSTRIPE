...
async def test_user_a_cannot_see_user_b_invoices(
    ...
    await create_invoice(test_org2.id, voucher_number="RE-B-001")
...
async def test_user_a_cannot_get_user_b_invoice_by_id(
    ...
    invoice_b = await create_invoice(test_org2.id, voucher_number="RE-B-002")
...
async def test_user_a_cannot_get_user_b_customer_by_id(
    ...
    customer_b = await create_customer(test_org2.id, name="User B Customer")
...
async def test_user_a_cannot_submit_user_b_invoice(
    ...
    customer_b = await create_customer(test_org2.id, name="User B")
    invoice_b = await create_invoice(test_org2.id, customer_b.id)
    await create_iban(test_org2.id, customer_b.id)
    ...
    with pytest.raises(CollectionError, match="nicht gefunden"):
        await CollectionService.submit_collection(
            tenant_id=test_org.id,  # User A's tenant
...
async def test_dashboard_stats_scoped_to_tenant(
    ...
    for i in range(3):
        await create_invoice(test_org2.id, voucher_number=f"RE-B-{i}")
