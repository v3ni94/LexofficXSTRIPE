<?php
/**
 * Zentrale Produktfaktenquelle (Version 4.79, Masterprompt Backend 13.09.2026).
 *
 ...
 * nie Quellen, Serverpfade, Zugangsdaten oder interne Kundendaten. Der Produktpreis ist seit der Vorgabe des Betreibers vom
 * 13.09.2026 oeffentlich, immer mit Betrag, Steuerhinweis und Periode; Vergleichs- und Streichpreise bleiben gesperrt
 * (TARIF-07). Ein unbestaetigter Preis wuerde weggelassen, nie als 0 oder kostenlos ausgegeben.
 *
...
function product_facts(): array
{
    ...
    return [
        ...
        $f('tarif.preis', 'tarif', 'UNLIMITED START: 25,00 EUR netto je vier Wochen (28 Tage), zuzüglich gesetzlicher Umsatzsteuer; Einführungspreis', 'technisch_getestet', ['php-ionos/sql/schema.sql plans (price_cents 2500, period_days 28)', 'tools/pricing-check.php Abschnitt D'], true, 'Öffentlich seit Vorgabe des Betreibers vom 13.09.2026, immer mit Betrag, Steuerhinweis und Periode. Vergleichs- oder Streichpreis nie nennen (tarif.vergleichspreis).'),
        $f('tarif.vergleichspreis', 'tarif', 'Regulärer Preis laut Projektvorgabe 50,00 EUR netto je vier Wochen; als Vergleichs- oder Streichpreis öffentlich gesperrt', 'ungeklaert', ['CLAUDE.md Preise', 'docs/seo/02-faktenregister.md TARIF-07'], false, 'Wettbewerbsrechtliche Zulässigkeit eines Vergleichspreises ungeklärt; kein Datensatz zu 50,00 EUR in plans.', null),
