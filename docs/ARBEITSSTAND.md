...
Stand: 25.09.2026 (Deploybranch 4.78; 4.79 im Arbeitsbranch `backend/masterprompt-2026-09-13`, aufgesetzt auf f29eb06, nicht deployt). Der Prüfbranch `audit/2026-09-09-gesamtpruefung` (4.59 bis 4.63, Basis 66c59d5 = 4.58) wurde am 11.09.2026 auf
...
Umgesetzt als 4.79 im Branch `backend/masterprompt-2026-09-13` (zunächst 4.76, nach den Frontend-Versionen 4.76 und 4.77 und dem Statusseiten-Fix 4.78 umnummeriert und auf f29eb06 aufgesetzt): Stripe-Verbindungszustand mit SEPA-Fähigkeit
...
| 4.78 | Statusseite zeigte seit dem Ende des externen Cronjobs (20.09.2026) „Störung“ für Datenabgleich und Einzugsverarbeitung: Komponente `cron` wertete nur `cron.php`-Läufe aus. `_mon_check_cron()` nimmt mit aktiver Warteschlange den Herzschlag des Schedulers (`worker_heartbeats.pool = 'scheduler'`); Kategorie `scheduler_late`. Verifikation nach dem Deployment: Statusseite muss nach dem nächsten Monitoring-Lauf (bis 5 Minuten) „Betriebsbereit“ zeigen; statisch geprüft, kein Prüfstand für das Monitoring | siehe git log | ja |
| Betrieb 20.09.2026 | Externer Cronjob (cron-job.org „SMART Einzug DE“, alle 5 Minuten) auf Uptime-Check `health.php` umgewidmet; `cron.php` wird von außen nicht mehr aufgerufen (auf dem VPS ohnehin redundant, Scheduler und Worker planen selbst). Offener Punkt „externer Uptime-Check“ damit erledigt; Totmannschalter `monitoring.heartbeat_url` bleibt offen (braucht einen Dienst mit eingehendem Ping, z. B. healthchecks.io) | keine Codeänderung | entfällt |
...
- Repository-Variable `WEBHOSTING_APP_DEPLOY` ist ungeprüft. Externer Uptime-Check seit 20.09.2026 eingerichtet (Betreiber, cron-job.org: der frühere Cronjob „SMART Einzug DE“ wurde auf `https://app.smart-einzug.de/health.php` umgewidmet, alle 5 Minuten, Benachrichtigung bei Fehlschlägen und ablaufendem TLS-Zertifikat; aus der Sitzung nicht einsehbar, Angabe des Betreibers). Der alte Cronjob gegen `cron.php` besteht damit nicht mehr; der VPS braucht ihn nicht (Scheduler und Worker).
...
Nachtrag 13.09.2026 (4.77, externe Durchsicht der Faktenseite): Drei Befunde. Behoben: Die Seite sagte „Belastet wird das Stripe-Konto Ihres eigenen Unternehmens“; belastet wird aber das Bankkonto des Zahlers, ueber das Stripe-Konto der Firma wird nur abgewickelt. Geaendert: „Jede Angabe ist gegen den Programmcode geprueft“ war fuer Leser nicht nachvollziehbar; die Seite trennt jetzt Aussagen ueber die eigene Software (gegen den Programmstand geprueft, verantwortlich die Mueller Holding AG) von Aussagen ueber Lexware Office und Stripe (deren Bedingungen sind verbindlich). Keine fremden Dokumentationsadressen erfunden, weil im Projekt keine belegt sind. ENTSCHEIDUNG DES BETREIBERS vom 13.09.2026: Der Produktpreis wird oeffentlich genannt; die Vorgabe vom 07.09.2026 ist damit aufgehoben. Preisseite und Faktenseite nennen 25,00 EUR netto je vier Wochen zzgl. USt mit rollierender Regel und Hinweis auf die Stripe-Gebuehren; Vergleichs- und Streichpreise bleiben gesperrt (TARIF-07 ungeklaert). `tools/pricing-check.php` Abschnitt D prueft seitdem die Richtigkeit der Angabe statt ihres Fehlens (Steuerhinweis, Periode, Preisseite fuehrt den Betrag, kein Streichpreis; Rechnungsbeispiele werden nicht mehr faelschlich getroffen), drei Gegenproben ausgefuehrt. CLAUDE.md entsprechend umgestellt.

Nachtrag 13.09.2026 (4.76, Masterprompt Frontend vom 13.09.2026): Neue oeffentliche Faktenseite `smart-einzug.de/fakten/` als Produktblatt, Inhalte ausschliesslich aus belegten Quellen (Faktenregister plus Gegenpruefung am Code), ohne Preisbetraege wegen der Betreibervorgabe vom 07.09.2026 und ohne Standortangabe, weil diese nicht belegbar ist. Verlinkt aus dem Fussbereich von 32 Seiten sowie aus Funktionen, Integrationen und Hilfe; Cluster C31 in der Keyword-Map. Berichtigt: Die Wissensseite zur Ruecklastschrift behauptete, nur der Webhook erkenne Ruecklastschriften; seit 4.73 erkennt sie auch der Statusabgleich. Derselbe Fehler steht noch im Faktenregister unter RUECK-01 und ist vom Backend nachzuziehen. Fuenf Berichte unter `docs/frontend/` angelegt und in die Marketingdokumentation aufgenommen. P0-Pruefung auf irrefuehrende Zahlungszustaende und Geheimnisse im Browser ohne Befund. Nicht pruefbar aus dieser Umgebung und im Testbericht als solches ausgewiesen: Abruf der Live-Seiten (Proxy verweigert fremde Domains), angemeldete Oberflaechen, Browsertests, Web Vitals als Felddaten.
Nachtrag 12.09.2026 (4.72): Auf Vorgabe des Betreibers tragen jetzt smart-einzug.de UND lexware-einzug.de das Google-Tag (AW-18431688840) im Kopf jeder Seite, in derselben Fassung und damit mit demselben Hash, gefolgt vom Ereignis-Schnipsel der Conversion „Kauf (1)". Auf lexware-einzug.de melden 52 Verweise auf die Registrierung die Conversion; ein Eingriff in `site.js` war nicht nötig, weil die Bindung nur prüft, ob die Seite den Schnipsel trägt. Datenschutzerklärung der Domain angepasst (Abschnitte 8a und 8c: Tag lädt beim Seitenaufruf, ohne Einwilligung keine Cookies). Offener Rechtspunkt im Faktenregister vermerkt: Die Leadseite gehört DETM, das Ads-Konto der Müller Holding AG; ob daraus eine gemeinsame Verantwortlichkeit nach Art. 26 DSGVO folgt, ist anwaltlich zu klären.

Nachtrag 12.09.2026 (4.71, Befund): Google Ads fand das Tag trotz sichtbarem Loader im Quelltext nicht. Ursache war nicht das Tag, sondern die Content-Security-Policy: `www.googleadservices.com` war nur in `script-src` erlaubt, also zum Laden des Skripts. Die Conversion meldet das Tag aber als Bild oder `fetch` an denselben Host, wofuer `img-src` und `connect-src` gelten; beide fuehrten ihn nicht, der Browser blockierte die Meldung. Der Fehler hat kein sichtbares Symptom: Die Seite laedt vollstaendig, nur die Messung kommt nie an. Ergaenzt sind jetzt `www.googleadservices.com`, `stats.g.doubleclick.net` und `td.doubleclick.net` in `img-src` und `connect-src` aller fuenf Marketingdomains; `tools/site-tag-check.py` prueft den Rueckkanal fuer jede Domain mit Ads-Kennung und ist gegen den alten Zustand rot. An der Einwilligung aendert sich nichts.
