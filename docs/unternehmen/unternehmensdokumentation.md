...
| Kennzahlen je Akquisitionsquelle, Firmen, Tarifpflege (seit 4.43 mit frei wählbarem Zeitraum: Tag, Woche, Monat, Quartal, Jahr, individuell, Vergleich zum Vorzeitraum) | Steuerung von Marketing und Produktangebot | Plattformadministrator | Plattformrolle mit `companies.view`, `companies.plan`, `plans.manage`; aktive 2FA | produktiv nachgewiesen | keine Umsatzkennzahlen im engeren Sinn, siehe Abschnitt 6 | `app/plans.php` | `php-ionos/admin.php` |
...
| Kundenprofil im Support (Firma und Benutzer: Stammdaten, Anschrift, Telefonnummern, Tarif, Verbindungen, Anfragen, Protokoll; Kontaktdaten pflegen) | Auskunft und Datenpflege ohne Wechsel in den Kundenaccount | Plattformrolle mit `support.view`, Ändern `support.customers` (Administrator, Mitarbeiter Support) | Grund je Änderung | im Quellcode vorhanden (4.62) | nur Kontaktdaten; Gläubiger-ID, SEPA-Einstellungen, Tarif, E-Mail-Adresse und Zugangsdaten bleiben der Firma vorbehalten; Sicherheitsmail an Inhaber oder Benutzer | `app/customer_profile.php`, Migration 033 | `php-ionos/admin-kunde.php` |
| Marketingmodul (Listen aus Firmenaccounts und CSV-Import mit Rechtsgrundlage, dauerhafte Sperrliste, Kampagnen im Design der Systemmails, Vorschau, Testversand, Freigabe mit 2FA-Code, Ratenbegrenzung je Sekunde und je 24 Stunden, Abmeldung per Link und One-Click, Rückläufer aus Amazon SES) | Werbe- und Informationsnachrichten an Kunden und Geschäftskontakte ohne Fremdwerkzeug und ohne Belastung der Systemmail-Reputation | Plattformrolle mit `marketing.view` (lesen) und `marketing.manage` | eigenes Versandprofil `mail_marketing` (Amazon SES), Testversand vor jeder Freigabe | im Quellcode vorhanden (4.63), SES-Einrichtung durch den Betreiber offen | Rechtsgrundlage je Import dokumentiert der Betreiber; SES-Sandbox begrenzt auf 200 je Tag | Migration 034, `app/marketing.php` | `php-ionos/admin-marketing.php`, `docs/marketing.md` |
