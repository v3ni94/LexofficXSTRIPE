...
| Ratenbegrenzung | Redis-Zähler je Sekunde (api_call_gate) je API-Schlüssel, also je Firma (jede Firma nutzt ihr eigenes Lexware- und Stripe-Konto), plus konfigurierbare Obergrenze über alle Firmen (lexoffice_global_per_second, stripe_global_per_second); zusätzlich die vorhandene Drosselung im Client | Ohne Redis greift nur die Drosselung je Prozess. Der Wert 2 Aufrufe je Sekunde je Schlüssel für Lexware ist eine zu verifizierende Annahme. Ein Rate-Limit einer Firma zählt nicht für den Circuit Breaker. |
...
- Cron (D-08, F-12): `cron.php` sichert sich mit `GET_LOCK('smarteinzug_cron_<hash der Datenbank>', 0)` gegen überlappende Läufe.
- Entscheidung (F-15): Nach einer Fairness-Abgabe behält ein manuell gestarteter Sync keine Bevorzugung mehr; wartende
  Jobs anderer Firmen gehen vor.
