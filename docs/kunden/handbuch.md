...
Im Kundendetail (Abschnitt "SEPA-Einzug") oder auf "Rechnungen" lässt sich SEPA-Einzug für einen Kunden auf "Nein" setzen. Dies gilt für alle Rechnungen mit dieser Kundennummer; solche Rechnungen lassen sich dann nicht mehr per Lastschrift einziehen, bis SEPA-Einzug wieder auf "Ja" gesetzt wird. In der Rechnungsliste zeigt die Spalte "Kunde" den aktuellen Zustand als Kennzeichen ("SEPA: Ja" grün, "SEPA: Nein" rot, "Laufkunde" ohne Kennzeichen); die Schaltfläche in der Spalte "Aktion" nennt die Änderung, die sie auslöst ("SEPA auf Nein setzen" beziehungsweise "SEPA auf Ja setzen").

Auf "Rechnungen" sind Rechnungen von Kunden mit "SEPA: Nein" standardmäßig ausgeblendet, damit die Liste nur enthält, was eingezogen werden kann. Die Schaltflächenreihe "Kunden" schaltet die Ansicht um: "Nur SEPA-Kunden" (Standard), "Nur SEPA: Nein" (zur Kontrolle der ausgeschlossenen Kunden) und "Alle Kunden" (alle offenen Rechnungen unabhängig vom SEPA-Kennzeichen). Unabhängig davon wechselt "Auch bezahlte anzeigen" zwischen offenen und allen Rechnungen; beide Filter lassen sich kombinieren und bleiben nach einer Aktion in der Liste erhalten. Rechnungen ohne zugeordneten Kunden (Kontakt in Lexware Office nicht gefunden) werden immer angezeigt, weil für sie kein SEPA-Kennzeichen existiert; für Laufkunden mit Sammel-Kundennummer lässt sich SEPA-Einzug nicht auf "Nein" setzen.
...
| **In Bearbeitung** | Stripe hat die SEPA-Lastschrift entgegengenommen; die Bank des Kunden verarbeitet sie noch. | Warten. "Status mit Stripe abgleichen" prüft den aktuellen Stand (nur Lesezugriff) und zugleich die abgeschlossenen Einzüge der letzten 70 Tage auf Rücklastschriften, oder auf den Stripe-Webhook warten. |
...
Der Statusabgleich ("Status mit Stripe abgleichen") prüft laufende Einzüge nur lesend; es bewegt sich dabei kein Geld. Seit Version 4.73 prüft er zusätzlich alle erfolgreichen und erstatteten Einzüge der letzten 70 Tage auf Rücklastschriften und Erstattungen (SEPA-Rücklastschriften können bis zu acht Wochen nach der Belastung eintreffen). Der Stripe-Webhook (Kapitel 5.3) meldet dasselbe ohne Ihr Zutun und ist der Regelweg; der Abgleich ist Ihre Kontrolle, falls eine Meldung ausbleibt. Eine erkannte Rücklastschrift setzt den Einzug auf "Zurückgebucht", die Rechnung erhält Klärungsbedarf und wird nicht automatisch erneut eingezogen.
