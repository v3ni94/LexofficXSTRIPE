...
| Serverskripte | setup-vps.sh, deploy.sh (Sperre, Symlink-Release, Migration per CLI, Reload, Worker-Neustart [Stand Auftrag III; seit Version 4.11 durch die Verifikation der Release-Bindung ersetzt], Health Check, Rollback), rollback.sh, db-import.sh, db-verify.php, maintenance.sh, backup.sh, restore-test.sh | deploy/vps/scripts, deploy/vps/backup |
...
| Deployment | source .env im Skript, Rollback aus deploy.sh scheiterte an der eigenen Sperre, kein Stop-Grace für laufende Jobs, Rollback ohne Schemaprüfung, Image-Prüfsumme nach Rollback veraltet | envval ohne source, Sperre vor Rollback freigegeben, stop_grace_period und restart -t 660 (Stand Auftrag III; seit Version 4.11 stop_signal SIGTERM mit 75 s Grace, Signalmodell in app/worker_signals.php und kein zweiter Neustart mehr), Verträglichkeitsprüfung des Migrationsstands (FORCE_ROLLBACK), Prüfsumme nach Build geschrieben |
