...
Stand 13.09.2026, Basis-Commit e29e5d8 (4.75), Arbeitsbranch `backend/masterprompt-2026-09-13` (4.76). Dieses Dokument
...
- Fremde Änderungen: Der Frontend-Chat hat am 12.09.2026 die Versionen 4.69 bis 4.72 (Google-Tag) über PR #6 und #7 in den
  Deploybranch gebracht; sie sind Bestandteil der Basis. Keine uncommitteten fremden Änderungen im Klon.
...
- Stripe je Firma: Secret oder Restricted Key verschlüsselt, Webhook-Secret je Firma, Konto-ID und Modus gespeichert;
  seit 4.76 zusätzlich Zahlungsfreischaltung und SEPA-Fähigkeit (`docs/backend/stripe-review.md`).
...
|---|---|---|---|
| MP-01 | P1 | Stripe-Verbindung galt als „verbunden“, sobald `GET /v1/account` antwortete; Zahlungsfreischaltung und SEPA-Fähigkeit blieben ungeprüft, ein Einzug scheiterte erst bei Stripe mit Rohtext (E-03) | behoben 4.76: Migration 035, `stripe_connection_state()`, Sperre in `_get_stripe_client($tenantId, true)`, Anzeige in `settings.php`; Prüfstand collections-check 16 (32 Fälle) |
| MP-02 | P2 | Webhook prüfte den Modus des Ereignisses (`livemode`) nicht gegen den Modus des hinterlegten Schlüssels (Signatur fängt den Fall meist ab) | behoben 4.76: Ereignis im falschen Modus wird mit 200 ignoriert; Fall 16a |
| MP-03 | P2 | Trennen der Stripe-Verbindung nannte nicht, dass terminierte Einzüge bestehen bleiben | behoben 4.76: Zahl der terminierten Einzüge im Hinweis und im Audit; Fälligkeitslauf stellt sie ohne Verbindung zurück (kein Fehlschlag, kein Einzug) |
| MP-04 | P1 | Keine zentrale, versionierte Produktfaktenquelle; Website und Faktenregister (`docs/seo`) ohne maschinenlesbaren Stand | behoben 4.76: `app/product_facts.php`, Snapshot, `fakten.php`, `tools/product-facts-check.php` 33/0 |
| MP-05 | P1 | Kein Datenvertrag zwischen Backend und Frontend | behoben: `docs/contracts/smarteinzug-contract.md` 1.0 |
| MP-06 | P3 | `billing-webhook.php` ohne Methodenprüfung (Signatur schlägt bei GET ohnehin fehl) | behoben 4.76: 405 für andere Methoden als POST |
