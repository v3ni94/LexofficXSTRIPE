...
Vertragsversion 1.0, Stand 13.09.2026. Basis-Commit e29e5d8 (4.75), Arbeitsbranch `backend/masterprompt-2026-09-13` (4.76).
...
ausgerollt ist; der Stand 4.76 liegt bis zur Freigabe nur im Arbeitsbranch.
...
2. **Endpunkt** `GET https://app.smart-einzug.de/fakten.php` (Version 4.76, öffentlich, ohne Login, ohne Sitzung, kein
...
Regeln: Preise stehen nicht im öffentlichen Snapshot, bis der Betreiber sie freigibt (Vorgabe 07.09.2026); ein fehlender
Preis ist nie als 0 EUR oder kostenlos darzustellen. `tarif.periode` (28 Tage, nicht Kalendermonat) und `tarif.stichtag`
(rollierender Satz aus `pricing.php`) sind öffentlich. Aussagen mit `status = oeffentlich_behauptet` dürfen nur mit der
...
**Stripe-Verbindung der Firma** (`stripe_connection_state()`, seit 4.76): `not_connected`, `disconnected`, `auth_failed`,
...
möglich; Statusabgleich, Webhook und Import lesen in jedem Zustand. `unverified` heißt: verbunden vor 4.76, Fähigkeiten
...
- Ausrollen von 4.76 (Migration 035) auf Staging und Abnahme des Verbindungszustands mit einem echten Stripe-Testkonto
...
- Freigabe der Preisangaben durch den Betreiber, danach `veroeffentlichen` für `tarif.preis` und Snapshot neu erzeugen.
