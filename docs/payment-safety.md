...
* `payment_intent.payment_failed` setzt `failed` mit Grund; `charge.dispute.created` setzt über `collection_apply_dispute()` `disputed`, Rechnung `failed` mit `requires_review = 1`, Audit `collection_disputed` (Feld `source`). Ein Neu-Einzug nach Rücklastschrift ist nur manuell möglich und läuft wieder durch alle Prüfungen (Restbetrag laut Lexware Office bleibt dabei maßgeblich; eine `disputed`-Collection zählt nicht als eigener Einzug).
* Seit 4.73 prüft "Status mit Stripe abgleichen" (`sync_collection_statuses()`) zusätzlich alle Einzüge im Status `succeeded` oder `refunded` der letzten `collections.sync_lookback_days` Tage (Vorgabe 70, höchstens 400; SEPA-Basislastschriften sind acht Wochen ohne Angabe von Gründen rückgabefähig). Statt je Einzug einen Abruf zu machen, liest der Abgleich `GET /v1/payment_intents?created[gte]=…&expand[]=data.latest_charge` seitenweise (100 je Seite, höchstens `COLLECTIONS_SYNC_MAX_PAGES` = 20 Seiten, sonst `truncated`) und wertet die eingebettete Charge aus: `disputed`/`dispute` führt zu `collection_apply_dispute()`, ein abweichender `amount_refunded` zu `collection_apply_refund()`. Beide Funktionen sind idempotent, die Wirkung ist identisch mit den Webhook-Ereignissen. PaymentIntents ohne Einzug der Firma werden übersprungen; die Liste stammt ohnehin aus dem Stripe-Konto der Firma. Der Webhook bleibt der Regelweg, der Abgleich ist die Kontrolle bei ausgebliebener Meldung. Prüffälle: `tools/collections-check.sh` Abschnitt 14d.
