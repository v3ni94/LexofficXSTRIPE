...
| lastschrift-einfach.de | Upload in `deploy.yml` | Ordner auf `.htaccess` (301 auf smart-abrechnen.de) und 404-Seite reduziert, aus allen Website-Werkzeugen entfernt | | Löschung der Altdateien auf dem Hoster (Upload ohne Löschoption) | umgesetzt, getestet |
...
| Anwendung | | `APP_VERSION` 4.32 mit Changelog-Eintrag für die Website-Änderungen (`php-ionos/app/version.php`); beim Merge mit dem Backend-Branch ist ein Konflikt in dieser Datei zu erwarten und von Hand aufzulösen | | | umgesetzt, getestet (`php -l`) |
...
4. M4 ist umgesetzt (Entscheidung vom 08.09.2026). Nach Inbetriebnahme: Search Console je Domain einrichten, Weiterleitungen und Indexierung prüfen; auf dem Hoster die Altdateien der weitergeleiteten Seiten und von lastschrift-einfach.de einmalig löschen, weil der Upload keine Dateien entfernt.
5. Backend: sevdesk-Domains in `signup_domains` der Produktionskonfiguration ergänzen; `APP_VERSION` 4.32 und der Changelog-Eintrag liegen in diesem Branch und müssen beim Merge gegen die Versionsnummern des Backend-Branchs abgestimmt werden (Konflikt in `version.php` erwartet).
