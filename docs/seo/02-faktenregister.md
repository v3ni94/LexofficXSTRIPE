...
**STATUS-03** (bestätigt): Statusabgleich ohne Webhook ('Status mit Stripe abgleichen', auch per Cron): Für Einzüge im Status 'processing' wird der PaymentIntent abgerufen; 'succeeded' setzt Erfolgreich und die Rechnung auf Eingezogen und speichert Charge und Stripe-Mandatsdaten; 'canceled' oder 'requires_payment_method' setzt Fehlgeschlagen mit Stripe-Fehlertext. Rücklastschriften (Disputes) und Erstattungen zu bereits erfolgreichen Einzügen wurden vom Abgleich bis 4.68 nicht erkannt. Seit 4.73 (12.09.2026) prüft der Abgleich zusätzlich erfolgreiche und erstattete Einzüge der letzten 70 Tage (collections.sync_lookback_days) über die Liste der PaymentIntents mit eingebetteter Charge und vermerkt Rücklastschrift und Erstattung wie der Webhook; der Webhook bleibt der Regelweg ohne Nutzeraktion, der Abgleich ist eine manuelle Kontrolle (kein Hintergrundlauf).  
...
**RUECK-01** (bestätigt, Stand 4.79): Eine Rücklastschrift oder ein Widerspruch des Zahlers erreicht die App als Stripe-Ereignis charge.dispute.created oder wird seit 4.73 vom manuellen Statusabgleich (Rückschau auf abgeschlossene Einzüge der letzten 70 Tage, gemeinsame Funktion collection_apply_dispute()) nachgeholt. Der Einzug erhält Status 'disputed' mit dem festen Grund 'SEPA-Lastschrift wurde vom Kunden widerrufen', die Rechnung wird 'failed', es entsteht ein Audit-Eintrag 'collection_disputed'. Der Stripe-Rückgabegrund (dispute reason) wird nicht ausgewertet; Rückgaben wegen mangelnder Deckung und echte Widersprüche werden gleich behandelt. Ein 'disputed'-Einzug zählt nicht mehr als eigener Einzug der Rechnung.  
