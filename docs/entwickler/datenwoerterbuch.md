...
| [integrations](#integrations) | Verbindungsdaten je Firma zu Lexware Office und Stripe (verschluesselte Zugangsdaten, Verbindungsstatus) sowie Wahl der aktiven Rechnungsquelle (Migration 024). | Externe Anbindungen | tenant_id | 30 | 1 |
...
| stripe_charges_enabled | TINYINT(1) | ja |  | Ergebnis der letzten Kontopruefung (GET /v1/account charges_enabled): 1 Zahlungen freigeschaltet, 0 nicht, NULL unbekannt (Migration 035, 4.79) |
| NULL | = unbekannt stripe_sepa_capability          VARCHAR(16)  NULL | ja |  |  |
| stripe_verify_error | VARCHAR(16) | ja |  | Fehlerklasse der letzten fehlgeschlagenen Pruefung einer bestehenden Verbindung: auth (401) \| permission (403) \| technical; NULL nach erfolgreicher Pruefung (integration_stripe_verify_failed(), Migration 035) |
| NULL | = letzte Pruefung erfolgreich (Migration 035) stripe_disconnected_at          DATETIME   NULL | ja |  |  |
...
**Migrationen:** Basistabelle vor Migrationszaehlung, 004_integration_verification.sql (vermutlich *_last_verified_at/_disconnected_at, *_company_name/_business_name/_mode, dem Namen nach passend), 024_invoice_source_switch.sql (invoice_source_changed_at, invoice_source_switches), 028_sevdesk_verbindung.sql, 035_stripe_verbindungszustand.sql (stripe_charges_enabled, stripe_sepa_capability, stripe_verify_error)  
...
**Verändert durch:** app/collections.php collections_cancel_all_pending(), collection_attempts_resolve(), store_stripe_mandate_data() (stripe_charge_id), _submit_collection_locked() (prenotified_at), cancel_scheduled_collection(), reschedule_collection() (scheduled_date, submit_not_before, queued_immediate, prenotified_at), sync_collection_statuses() (seit 4.73 auch Rueckschau auf abgeschlossene Einzuege), collection_apply_dispute() (stripe_status disputed, failure_reason, completed_at, stripe_charge_id; aus Webhook und Statusabgleich), collection_apply_refund() (refunded_cents, refunded_at, refund_note, stripe_status), process_scheduled_collections() (note, stripe_status, failure_reason), _submit_single_scheduled() (stripe_status, customer_iban_id, mandate_id, amount_cents, note), stripe-webhook.php (stripe_status, completed_at anhand Stripe-Ereignis)  
