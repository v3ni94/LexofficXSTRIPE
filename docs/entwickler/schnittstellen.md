...
| `admin-kunde.php` (seit 4.62) | Kundenprofil: Firma oder Benutzer mit Stammdaten, Anschrift, Telefonnummern, Tarif, Verbindungen, Mitgliedern, Anfragen, Sitzungen, Protokoll; Kontaktdaten pflegen | GET, POST | `require_platform('support.view')` | Plattformrolle mit `support.view`; Ändern `support.customers` plus Grund (`app/customer_profile.php`), gesperrte Felder siehe `docs/entwickler/sicherheit.md` | GET `org`, `user`; POST `action` (`org_update`, `user_update`), `org_id`, `user_id`, `reason`, `name`, `street`, `zip`, `city`, `country`, `display_name`, `first_name`, `last_name`, `phone_private`, `phone_business` | HTML | Admin |
| `admin-marketing.php` (seit 4.63) | Marketing: Listen (CSV-Import, Systemliste), Sperrliste, Kampagnen mit Vorschau, Testversand, Freigabe, Ratenbegrenzung, Ereignisse | GET, POST | `require_platform('marketing.view')` | Ändern `marketing.manage`; `campaign_start` zusätzlich 2FA-Code (`docs/marketing.md`) | GET `liste`, `kampagne`, `vorschau`, `export`, `q`, `sq`; POST `action` (`rates_save`, `list_create`, `list_import`, `list_sync`, `list_delete`, `recipient_remove`, `suppress_add`, `suppress_remove`, `campaign_save`, `campaign_test`, `campaign_start`, `campaign_pause`, `campaign_resume`, `campaign_cancel`, `campaign_delete`), Datei `csv` | HTML, CSV | Admin |
| `abmelden.php` (seit 4.63) | Abmeldung von Werbenachrichten über Token aus der Nachricht (Seite mit Schaltfläche, POST, One-Click nach RFC 8058) | GET, POST | Token `t` (48 Hexzeichen, nur als Hash gespeichert) | öffentlich, kein Sitzungs-CSRF (Aufruf aus dem Postfach) | GET/POST `t`; POST `aktion=abmelden` oder `List-Unsubscribe=One-Click` | HTML, bei One-Click text/plain | App |
| `marketing-webhook.php` (seit 4.63) | Rückläufer und Beschwerden aus Amazon SES über SNS (SubscriptionConfirmation, Notification) | POST | `mail_marketing.webhook_token` in der Adresse UND SNS-Signaturprüfung gegen Zertifikat von `sns.<region>.amazonaws.com` | Amazon SNS | GET `token`; Body JSON der SNS-Nachricht | text/plain, 200/400/403/404/405/413/500/502 | App |
...
### `fakten.php` (öffentliche Produktfakten, seit 4.79)
...
Seit 4.79 wird nach der Signaturprüfung `event.livemode` mit `integrations.stripe_mode` der Firma verglichen; ein
Ereignis im falschen Modus wird mit 200 ignoriert (`webhook_exit`, Altbestand ohne `stripe_mode` wird nicht verglichen).
`billing-webhook.php` antwortet auf andere Methoden als POST mit 405 (4.79).
