...
| `admin-legal.php` | Adminbereich: Rechtsdokumente verwalten/veröffentlichen | GET, POST | `require_superadmin()` (`admin-legal.php:14`) | Plattformadministrator, Veröffentlichen und Zurückziehen zusätzlich frischer 2FA-Code (Entwürfe ohne, seit 4.36) | GET `dok`; POST `action`, `doc_code`, `title`, `version`, `summary`, `body_md`, `required_for`, `draft`, `document_id`, `code` | HTML | Admin |
