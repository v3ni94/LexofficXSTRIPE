...
- `require_onboarded()` (`app/auth.php:221`), `require_subscription()` (`app/auth.php:235`),
  ...
  `require_superadmin()` (`app/auth.php:277`, verlangt zusätzlich `is_superadmin = 1` und aktive 2FA).
...
| `admin-legal.php` | Adminbereich: Rechtsdokumente verwalten/veröffentlichen | GET, POST | `require_superadmin()` (`admin-legal.php:14`) | Plattformadministrator, kritische Aktionen zusätzlich frischer 2FA-Code (Doku-Kopf) | GET `dok`; POST `action`, `doc_code`, `title`, `version`, `summary`, `body_md`, `required_for`, `draft`, `document_id`, `code` | HTML | Admin |
| `admin-support.php` | Superadmin Support: Firmenwechsel, Sitzungen, Entsperren, 2FA-Reset, Tickets | GET, POST | `require_superadmin()` (`admin-support.php:20`) | Plattformadministrator, Firmenzugriff zusätzlich Begründung + frischer 2FA-Code (`app/support.php:21`) | GET `q`, `ticket`, `tstatus`; POST `action`, `org_id`, `reason`, `code`, `email`, `password`, `session_id`, `ticket_id`, `body` | HTML | Admin |
| `admin-system.php` | Betriebsübersicht (Monitoring, Warteschlange, Störungsmeldungen) | GET, POST | `require_superadmin()` (`admin-system.php:32`) | Ansicht: jeder Superadmin; Ändern von Überwachungseinstellungen/Störungsmeldungen/Testversand: zusätzlich `monitoring.editors` + frischer 2FA-Code (Doku-Kopf) | GET `tab`, `d`, `w`; POST `action`, `code`, `org_id`, `job_id`, `kind`, `reason`, `components`, `title`, `public_message`, `public_text`, `internal_note(s)`, `phase`, `started_at`, `scheduled_end_at`, `incident_id` | HTML | Admin |
