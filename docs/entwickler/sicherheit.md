...
**Konfiguration.** Die Ads-Kennung `AW-18431688840` steht seit 4.58 als Vorgabe im Code (`TRACKING_DEFAULT_ADS_ID`), wie schon in der `site.js` der Marketingseiten. Sie ist kein Geheimnis, weil sie ohnehin im Seitenquelltext steht, und wirkt damit mit dem Deployment ohne Eingriff in `shared/config.php`. Der Block `analytics` in der Konfiguration ist optional: `ads_id` überschreibt die Vorgabe, `ga_id` schaltet Analytics zu, `enabled` auf `false` schaltet Tag und Banner vollständig ab. Analytics bleibt ohne Konfiguration aus, weil es für app.smart-einzug.de keine eigene GA4-Property gibt; eine Kennung einer Marketingdomain würde die Messwerte vermischen. Alle Kennungen werden gegen die von Google vergebenen Formate geprüft (`G-…`, `AW-…`), damit eine falsch gepflegte Konfiguration keine fremde Kennung einschleust.
