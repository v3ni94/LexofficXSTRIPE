...
// Preisbetraege sind bis zur Freigabe gesperrt: im oeffentlichen Snapshot darf kein Betrag zu tarif.preis stehen
$preisPublic = array_values(array_filter($public, static fn($p) => $p['key'] === 'tarif.preis'));
$preisIntern = array_values(array_filter($facts, static fn($p) => $p['key'] === 'tarif.preis'));
($preisIntern && !$preisIntern[0]['veroeffentlichen'] && !$preisPublic) ? $ok('tarif.preis intern gefuehrt, oeffentlich gesperrt (Vorgabe 07.09.2026)') : $bad('tarif.preis Veroeffentlichung abweichend von der Vorgabe');
preg_match('/\d+,\d{2}\s*eur/u', $flat) ? $bad('Preisbetrag im oeffentlichen Snapshot') : $ok('kein Preisbetrag im oeffentlichen Snapshot');
...
(str_contains((string)$by['tarif.periode']['wert'], "$days Tage") && str_contains((string)$by['tarif.preis']['wert'], "je $days Tage")) ? $ok("Periode $days Tage") : $bad('Periode weicht von plans.period_days ab');
