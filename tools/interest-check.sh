...
! sed -n '/^function interest_register/,/^function interest_pending_mail_count/p' "$ROOT/php-ionos/app/interest.php" | grep -q "mail_enabled()\|status = 'confirmed'\|'confirmed', " && grep -q "mail_deferred" "$ROOT/php-ionos/app/interest.php" && ok "keine stille Bestaetigung ohne Mailversand; nicht sendbare Mails werden als wartend markiert" || bad "interest_register bestaetigt still oder kennt kein mail_deferred"
