...
echo "5) Kopfzeilenkodierung (4.64, RFC 2047 an Wortgrenzen)\n";
$dec = static fn(string $enc): string => mb_decode_mimeheader($enc);
$ascii = 'Willkommen bei SmartEinzug';
(mail_encode_header_value($ascii) === $ascii) ? $ok('ASCII bleibt unkodiert') : $bad('ASCII kodiert');
$s1 = 'Willkommen bei SmartEinzug: Bitte E-Mail-Adresse bestätigen';
$e1 = mail_encode_header_value($s1);
($dec($e1) === $s1) ? $ok('Betreff mit Umlaut verlustfrei dekodierbar') : $bad('Dekodierung: ' . $dec($e1));
(!str_contains($e1, '?Q?') && str_starts_with($e1, '=?UTF-8?B?')) ? $ok('Base64 statt Q-Kodierung') : $bad('Kodierung: ' . $e1);
$zeilen = explode("\r\n", $e1);
(max(array_map('strlen', $zeilen)) <= 76 && count(array_filter(array_slice($zeilen, 1), static fn($z) => !str_starts_with($z, ' '))) === 0) ? $ok('Zeilen hoechstens 76 Zeichen, Fortsetzungen eingerueckt') : $bad('Faltung: ' . json_encode($zeilen));
$teile = array_map(static fn($z) => base64_decode(substr(trim($z), 10, -2)), $zeilen);
(count(array_filter($teile, static fn($t) => $t !== '' && !str_ends_with($t, ' ') && $t !== end($teile))) === 0) ? $ok('Trennung nur an Wortgrenzen (jeder Teil endet mit Leerzeichen)') : $bad('Teile: ' . json_encode($teile));
(count(array_filter($teile, static fn($t) => !mb_check_encoding($t, 'UTF-8'))) === 0) ? $ok('kein Multibyte-Zeichen getrennt') : $bad('UTF-8 verletzt');
$lang = str_repeat('ä', 80);
$e2 = mail_encode_header_value($lang);
($dec($e2) === $lang && max(array_map('strlen', explode("\r\n", $e2))) <= 76 && count(array_filter(array_map(static fn($z) => base64_decode(substr(trim($z), 10, -2)), explode("\r\n", $e2)), static fn($t) => !mb_check_encoding($t, 'UTF-8'))) === 0) ? $ok('ueberlanges Wort zeichenweise gefaltet, UTF-8 intakt') : $bad('langes Wort: ' . $e2);
$pn2 = mail_tpl_prenotification(['name' => 'Müller & Söhne GmbH', 'creditor_identifier' => ''], ['voucher_number' => 'RE-2026-0042'], ['mandate_reference' => 'M-1'], 100, '2026-09-22');
($dec(mail_encode_header_value($pn2['subject'])) === $pn2['subject']) ? $ok('Betreff der Vorabankuendigung verlustfrei') : $bad('Vorabankuendigung Betreff');
(mail_encode_header_value("Zeile\r\nBcc: x") === 'ZeileBcc: x') ? $ok('Zeilenumbrueche im Wert entfernt (Header-Injection)') : $bad('Injection: ' . mail_encode_header_value("Zeile\r\nBcc: x"));
$GLOBALS['config']['mail'] = ['enabled' => true, 'transport' => 'log', 'log_file' => $logFile, 'from_address' => 'kontakt@smart-einzug.de', 'from_name' => 'SmartEinzug'];
mail_send_direct('empfaenger@example.test', $s1, "Text\n");
$log = (string)@file_get_contents($logFile); @unlink($logFile);
preg_match('/^Subject: ([^\r\n]*(?:\r\n[ \t][^\r\n]*)*)/m', $log, $sm);
(isset($sm[1]) && mb_decode_mimeheader($sm[1]) === $s1 && !str_contains($sm[1], '?Q?')) ? $ok('Transport log: Betreff als Base64-Woerter, dekodiert identisch') : $bad('Transport log Betreff: ' . ($sm[1] ?? '-'));
unset($GLOBALS['config']['mail']['enabled']);
